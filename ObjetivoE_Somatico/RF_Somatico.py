...
# Carpeta para resultados de Random Forest en el objetivo somático
...
def main():
    ...
    # ======================================================
    #  CSV RESUMEN GLOBAL DEL MODELO (DIMENSIÓN SOMÁTICA)
    # ======================================================
    df_global_modelo = pd.DataFrame(
        [
            {
                "Modelo": "RandomForest_OE2",
                "Accuracy": round(acc, 3),
                "BalancedAccuracy": round(bal_acc, 3),
    ...
    ruta_global = DIR_OUT / "metricas_oe2_rf_modelo.csv"
    df_global_modelo.to_csv(ruta_global, index=False, encoding="utf-8-sig")
    print(f"[OK] Métricas globales del modelo (RF_OE2) guardadas en:", ruta_global)
