...
RUTA_CURVA_PERDIDA_IMG     = DIR_OUT / "curva_perdida_gbm.png"
RUTA_MATRIZ_CONFUSION      = DIR_OUT / "matriz_confusion_gbm.png"
RUTA_CURVA_ROC_IMG         = DIR_OUT / "curva_roc_gbm.png"
...
RUTA_METRICAS_CSV          = DIR_OUT / "metricas_modelo_gbm.csv"
...
# =========================
# HIPERPARÁMETROS GBM (Estrategia ANTI-OVERFITTING FUERTE)
# =========================
# Hemos endurecido las reglas para cerrar la brecha entre train y validation.
GBM_PARAMS = dict(
    max_depth=2,            
    min_samples_leaf=15,    
...
def construir_modelo_gbm(X_train, y_train, cv, ruta_png, ruta_csv):
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de Aprendizaje de Gradient Boosting", fontsize=16, weight="bold")
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de Pérdida de Gradient Boosting", fontsize=16, weight="bold")
    plt.xlabel("Tamaño del conjunto de entrenamiento (TRAIN)", fontsize=14)
...
def main():
    ...
    resumen_gbm = (
        f"Gradient Boosting — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}, " # <--- F1 Primero
        f"Precisión = {prec_macro:.2f}, Sensibilidad = {rec_macro:.2f}, "
        f"ROC-AUC = {roc_auc_macro:.2f}" if roc_auc_macro is not None
        else f"Gradient Boosting — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}..."
    )
    ...
    ax.set_title("Matriz de confusión: \nGradient Boosting", fontsize=16, weight="bold")
    ...
    plt.title('Curva ROC Multiclase — Gradient Boosting', fontsize=14, weight='bold')
...
# ==========================================================
    ...
    RUTA_ROC_JSON = DIR_OUT / "roc_data_gbm.json"
