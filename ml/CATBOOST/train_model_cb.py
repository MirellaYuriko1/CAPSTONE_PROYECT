...
RUTA_CURVA_IMG             = DIR_OUT / "curva_entrenamiento_validacion_cb.png"
RUTA_CURVA_CSV             = DIR_OUT / "curva_entrenamiento_validacion_cb.csv"
RUTA_CURVA_PERDIDA_IMG     = DIR_OUT / "curva_perdida_cb.png"
RUTA_MATRIZ_CONFUSION      = DIR_OUT / "matriz_confusion_cb.png"
RUTA_CURVA_ROC_IMG         = DIR_OUT / "curva_roc_cb.png"
RUTA_METRICAS              = DIR_OUT / "metricas_cb.json"
RUTA_REPORTE_CLASIFICACION = DIR_OUT / "reporte_clasificacion_cb.txt"
RUTA_MODELO                = DIR_OUT / "modelo_cb.pkl"
RUTA_PARAMS                = DIR_OUT / "parametros_cb.json"

# --- NUEVA RUTA PARA EL CSV RESUMEN ---
RUTA_METRICAS_CSV          = DIR_OUT / "metricas_modelo_cb.csv"
...
CB_PARAMS = dict(
    ...
    eval_metric="MultiClass", # Métrica interna de optimización (Log Loss)
    random_seed=SEED,
...
def construir_modelo_catboost(X_train, y_train, cv, ruta_png, ruta_csv):
    ...
    # ---- Plot F1-Score Macro (CAMBIO VISUAL) ----
    ...
    plt.ylim(0, 1)
    plt.title("Curva de Aprendizaje de CatBoost", fontsize=16, weight="bold")
    ...
    plt.close()
    print("[OK] Curva ENTRENAMIENTO/VALIDACIÓN (F1) guardada en:", ruta_png)

    # ---- Plot Pérdida (Inversa del F1) ----
    plt.figure(figsize=(7.8, 5.6))
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de pérdida de CatBoost", fontsize=16, weight="bold")
    plt.xlabel("Tamaño del conjunto de entrenamiento (TRAIN)", fontsize=14)
    ...
    plt.close()
    print("[OK] Curva de ERROR (1-F1) guardada en:", RUTA_CURVA_PERDIDA_IMG)
...
def main():
    ...
    # Curva de aprendizaje (Usando F1-Macro internamente)
    sizes_abs, tr_mean, va_mean, tr_std, va_std = construir_modelo_catboost(
        X_train, y_train, cv, RUTA_CURVA_IMG, RUTA_CURVA_CSV
    )

    # Rendimiento promedio en CV-5 (SOLO TRAIN - Métrica F1-Macro)
    # <--- CAMBIO: cross_val_score ahora usa 'f1_macro'
    cv_model = CatBoostClassifier(**CB_PARAMS)
    cv_metric = cross_val_score(cv_model, X_train, y_train, cv=cv, scoring="f1_macro", n_jobs=-1)
    ...
    y_pred = np.array(y_pred_raw).ravel().astype(int)
    ...
    print("\n--- RESUMEN CATBOOST (TEST HOLD-OUT 20%) ---")
    resumen_cb = (
        f"CatBoost — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}, " # <--- F1 Puesto primero
        f"Precisión = {prec_macro:.2f}, Sensibilidad = {rec_macro:.2f}, "
        f"ROC-AUC = {roc_auc_macro:.2f}" if roc_auc_macro is not None
        else f"CatBoost — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}..."
    )
    print(resumen_cb)
    ...
    ax.set_title("Matriz de confusión: CatBoost", fontsize=16, weight="bold")
    ...
    plt.title('Curva ROC Multiclase — CatBoost', fontsize=14, weight='bold')
    ...
    with open(RUTA_PARAMS, "w", encoding="utf-8") as f:
        json.dump({"params": CB_PARAMS_SERIALIZABLE, "features_used": FEATURES}, f, ensure_ascii=False, indent=2)
    print("[OK] Modelo guardado en:", RUTA_MODELO)
    print("[OK] Parámetros guardados en:", RUTA_PARAMS)

    # ======= COPIA PARA DESPLIEGUE =======
...
# ==========================================================
    ...
    RUTA_ROC_JSON = DIR_OUT / "roc_data_cb.json"
