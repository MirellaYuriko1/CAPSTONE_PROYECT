...
def construir_modelo_catboost(X_train, y_train, cv, ruta_png, ruta_csv):
    ...
    # ---- Plot Accuracy (con clipping como RF) ----
    eps = 1e-3
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de pérdida de CatBoost")
    plt.xlabel("Tamaño del conjunto de entrenamiento (TRAIN)")
    plt.ylabel("Pérdida")
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    # Curva de aprendizaje (TRAIN y VALID con CV-5) + CSV + PÉRDIDA
    ...
    resumen_cb = (
        f"CatBoost — Accuracy = {acc_test:.2f}, Precisión = {prec_macro:.2f}, "
        f"Sensibilidad = {rec_macro:.2f}, F1-Score = {f1m_test:.2f}, "
        f"ROC-AUC = {roc_auc_macro:.2f}" if roc_auc_macro is not None
        else f"CatBoost — Accuracy = {acc_test:.2f}, Precisión = {prec_macro:.2f}, "
    ...
    ax.set_title("Matriz de confusión: CatBoost", fontsize=14)
