...
XGB_PARAMS = dict(
    objective="multi:softprob",   # probas por clase
    num_class=5,
...
# ----------------------------------------------------------
# Curva de aprendizaje (SIN números, Y=0..1, exporta CSV)
# ----------------------------------------------------------
def curva_aprendizaje_xgb(X_train, y_train_0based, cv, ruta_png, ruta_csv):
    ...
    # ---- FIGURA (sin números en los puntos) ----
...
def main():
    ...
    # Curva de aprendizaje (SIN números, Y=0..1, CSV)
    sizes_abs, tr_mean, va_mean, tr_std, va_std = curva_aprendizaje_xgb(
        X_train, y_train_0, cv, RUTA_CURVA_IMG, RUTA_CURVA_CSV
    )

    # CV accuracy en TRAIN
    modelo_cv = XGBClassifier(**XGB_PARAMS)
    cv_acc = cross_val_score(modelo_cv, X_train, y_train_0, cv=cv, scoring="accuracy", n_jobs=-1)
