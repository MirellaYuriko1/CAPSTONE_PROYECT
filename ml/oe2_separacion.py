# ml/oe2_separacion.py
# ------------------------------------------------------------
# OE2 - Ansiedad por separación
# Principal: UPPERBOUND (usa SOLO los ítems de Dim2)
# Opcional : LOSO (excluye Dim2 y usa Dim1, Dim3..Dim6 + edad + género)
# Salidas: reports/oe2_separacion/{tables,figs,figs_ml,tables_ml}
# ------------------------------------------------------------
...
DIM_KEY = "Dim2"   # Ansiedad por separación
OUTDIR = Path(__file__).parent / "reports" / "oe2_separacion"
RUN_LOSO = False   # pon True si quieres generar también la variante LOSO

# ======= Textos para títulos y leyendas =======
UI = {
    "pred_title": "Gráfico comparativo entre diagnósticos predichos y diagnósticos esperados con",
    "pred_sub":   "respecto a la dimensión de Ansiedad por Separación",
    "leg_esp":    "Esperado",
    "leg_pred":   "Predicción",
    "x_nivel":    "Nivel",
    "y_n":        "Número de estudiantes",

    "cm_title":        "Matriz de confusión — Ansiedad por separación",
    "cm_title_norm":   "Matriz de confusión (porcentaje por fila) — Ansiedad por separación",
    "color_casos":     "Casos",
    "color_pct":       "% de casos",
    "x_pred":          "Predicho",
    "y_exp":           "Esperado",

    "roc_title": "Curvas ROC por nivel — Ansiedad por separación",
    "roc_x":     "Falsos positivos (FPR)",
    "roc_y":     "Verdaderos positivos (TPR)",

    "pr_title":  "Curvas Precisión–Recall por nivel — Ansiedad por separación",
    "pr_x":      "Recall",
    "pr_y":      "Precisión",

    "cal_title": 'Calibración de probabilidades — nivel "Muy alto"',
    "cal_leg1":  "Observado",
    "cal_leg2":  "Perfectamente calibrado",
    "cal_x":     "Probabilidad predicha",
    "cal_y":     "Fracción observada",

    "cv_title":  "Estabilidad en validación cruzada (k=5) — F1_macro",
    "cv_y":      "F1_macro",

    "thr_title": 'Ajuste de umbral — nivel "Muy alto"',
    "thr_x":     "Umbral de decisión",
    "thr_y":     "Valor",

    "imp_items_title": "Aporte de cada ítem (Dim2) — Ansiedad por separación",
    "imp_items_x":     "Importancia (permutación)",
    "imp_items_y":     "Ítems de la subescala",

    "learn_title": "Curva de aprendizaje — Random Forest (F1_macro)",
...
# ================ Helpers ================
def _bytes_to_int(x):
    if isinstance(x, (bytes, bytearray)): return int(x[0])
    return x
...
def _normaliza_genero(g):
    ...
    if s in ("f", "fem", "femenino", "female"): return "Femenino"
    if s in ("m", "masc", "masculino", "male"): return "Masculino"
    return str(g or "")
...
def cargar_dataset():
    ...
    # Etiqueta esperada SOLO de Dim2 (normas)
    y_esp = []
    for _, row in df.iterrows():
        sumas_dim = {dim: row[f"{dim}_sum"] for dim in DIMENSIONES.keys()}
        subesc, _tot = interpreta_normas(row.get("genero"), int(row.get("edad") or 0),
                                         sumas_dim, int(row["total_sum"]))
        nombre_sub = DIM_NOMBRES[DIM_KEY]  # nombre interno de Dim2
...
def evaluar_upper_rf(df):
    ...
    items_dim2 = [f"p{i}" for i in DIMENSIONES["Dim2"]]
    X = df[items_dim2].copy()
    y = df["y_expected"].copy()

    clf = Pipeline([
        ("pre", StandardScaler(with_mean=False)),
        ("mdl", RandomForestClassifier(n_estimators=400, random_state=RANDOM_STATE, class_weight="balanced"))
    ])
    ...
    # Tablas
    _pred_vs_esp_table(y, y_pred).to_csv(OUTDIR / "tables" / "pred_vs_esp_UpperBound_rf.csv", index=False, encoding="utf-8")
    ovr_counts_and_metrics(y, y_pred, LEVELS).to_csv(OUTDIR / "tables" / "tabla12_ovr_UpperBound_rf.csv", index=False, encoding="utf-8")
    filas = []
    for cls in LEVELS:
    ...
    filas.append({"Clase":"GLOBAL",
                  ...
                  "Brier": round(brier,3),
                  "n": int(len(y))})
    pd.DataFrame(filas).to_csv(OUTDIR / "tables" / "metricas_UpperBound_rf.csv", index=False, encoding="utf-8")
    ...
    plt.tight_layout(); plt.savefig(OUTDIR / "figs" / "pred_vs_esp_barras_UpperBound_rf.png"); plt.close()

    # Matriz de confusión (recuentos)
    ...
    for i in range(cm_norm.shape[0]):
        for j in range(cm_norm.shape[1]): plt.text(j, i, f"{cm_norm[i,j]*100:.0f}%", ha="center", va="center", fontsize=9)
    ...
    plt.figure(figsize=(6.2,4.6), dpi=160)
    plt.plot(ths, precs, marker="o", label="Precisión")
    plt.plot(ths, recs, marker="o", label="Recall")
    plt.xlabel(UI["thr_x"]); plt.ylabel(UI["thr_y"]); plt.title(UI["thr_title"])
    plt.legend(); plt.tight_layout(); plt.savefig(ml_dir / "umbral_muy_alto_UpperBound_rf.png"); plt.close()

    # Importancia por ítem (permutación)
    ...
    plt2.barh([items_dim2[i] for i in order], [r.importances_mean[i] for i in order])
    plt2.title(UI["imp_items_title"])
    plt2.xlabel(UI["imp_items_x"]); plt2.ylabel(UI["imp_items_y"])
    plt2.tight_layout(); plt2.savefig(ml_dir / "importancias_items_dim2_UpperBound_rf.png"); plt2.close()
...
def evaluar_loso_rf(df):
    ...
    (OUTDIR / "tables").mkdir(parents=True, exist_ok=True)
    (OUTDIR / "figs").mkdir(parents=True, exist_ok=True)
    _pred_vs_esp_table(y, y_pred).to_csv(OUTDIR / "tables" / "pred_vs_esp_LOSO_rf.csv", index=False, encoding="utf-8")
    cm = confusion_matrix(y, y_pred, labels=LEVELS)
    import matplotlib.pyplot as plt
    plt.figure(figsize=(5.6,4.6), dpi=160)
    im = plt.imshow(cm, aspect="auto")
