# ml/oe1_panico.py
# ------------------------------------------------------------
# OE1 - Pánico/Agorafobia
# Principal: UPPERBOUND (usa SOLO los 9 ítems de Dim1)
# Opcional : LOSO (excluye Dim1 y usa Dim2..Dim6 + edad + género)
# Salidas: reports/oe1_panico/{tables,figs,figs_ml,tables_ml}
# ------------------------------------------------------------
...
DIM_KEY = "Dim1"  # Pánico/Agorafobia
OUTDIR = Path(__file__).parent / "reports" / "oe1_panico"
...
UI = {
    ...
    "pred_sub":   "respecto a la dimensión de Ataques de Pánico y Agorafobia",
    "leg_esp":    "Esperado",
    "leg_pred":   "Predicción",
    "x_nivel":    "Nivel",
    "y_n":        "Número de estudiantes",

    "cm_title":        "Matriz de confusión — Pánico/agorafobia",
    "cm_title_norm":   "Matriz de confusión (porcentaje por fila) — Pánico/agorafobia",
    "color_casos":     "Casos",
    "color_pct":       "% de casos",
    "x_pred":          "Predicho",
    "y_exp":           "Esperado",

    "roc_title": "Curvas ROC por nivel — Pánico/agorafobia",
    "roc_x":     "Falsos positivos (FPR)",
    "roc_y":     "Verdaderos positivos (TPR)",

    "pr_title":  "Curvas Precisión–Recall por nivel — Pánico/agorafobia",
    "pr_x":      "Recall",
    "pr_y":      "Precisión",
    ...
    "imp_items_title": "Aporte de cada ítem (Dim1) — Pánico/agorafobia",
...
def cargar_dataset():
    ...
    for _, row in df.iterrows():
        ...
        nombre_sub = DIM_NOMBRES[DIM_KEY]  # "PanicoAgorafobia"
...
def evaluar_upper_rf(df):
    ...
    items_dim1 = [f"p{i}" for i in DIMENSIONES["Dim1"]]
    X = df[items_dim1].copy()
    ...
    for t in ths:
        ...
        precs.append(prec); recs.append(recall:=rec)
    plt.figure(figsize=(6.2,4.6), dpi=160)
    ...
    plt2.barh([items_dim1[i] for i in order], [r.importances_mean[i] for i in order])
    plt2.title(UI["imp_items_title"])
    plt2.xlabel(UI["imp_items_x"]); plt2.ylabel(UI["imp_items_y"])
    plt2.tight_layout(); plt2.savefig(ml_dir / "importancias_items_dim1_UpperBound_rf.png"); plt2.close()
