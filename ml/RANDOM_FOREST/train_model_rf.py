...
RUTA_CURVA_ROC_IMG         = DIR_OUT / "curva_roc_rf.png"
...
RUTA_METRICAS_CSV = DIR_OUT / "metricas_modelo_rf.csv"
...
# =========================
# HIPERPARÁMETROS RF (Estrategia Robusta)
# =========================
# max_depth=10: Evita árboles infinitos que memorizan (Overfitting).
# min_samples_leaf=5: Obliga a que cada decisión tenga sustento estadístico.
RF_PARAMS = dict(
    n_estimators=400,
    max_depth=8,          
    min_samples_leaf=10,     
...
def construir_modelo_rf(X_train, y_train, cv, ruta_png, ruta_csv):
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de Aprendizaje de Random Forest", fontsize=16, weight="bold")
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de Pérdida de Random Forest", fontsize=16, weight="bold")
    plt.xlabel("Tamaño del conjunto de entrenamiento (TRAIN)", fontsize=14)
...
def main():
    ...
    resumen_rf = (
        f"Random Forest — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}, " # <--- F1 Primero
        f"Precisión = {prec_macro:.2f}, Sensibilidad = {rec_macro:.2f}, "
        f"ROC-AUC = {roc_auc_macro:.2f}" if roc_auc_macro is not None
        else f"Random Forest — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}..."
    )
    print(resumen_rf)
    ...
    ax.set_title("Matriz de confusión: Random Forest", fontsize=16, weight="bold")
    ...
    plt.title('Curva ROC Multiclase — Random Forest', fontsize=14, weight='bold')
...
# ==========================================================
    ...
    RUTA_ROC_JSON = DIR_OUT / "roc_data_rf.json"
