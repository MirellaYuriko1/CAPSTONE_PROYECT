...
RUTA_CURVA_PERDIDA_IMG     = DIR_OUT / "curva_perdida_svm.png"
RUTA_MATRIZ_CONFUSION      = DIR_OUT / "matriz_confusion_svm.png"
RUTA_CURVA_ROC_IMG         = DIR_OUT / "curva_roc_svm.png"
...
RUTA_METRICAS_CSV = DIR_OUT / "metricas_modelo_svm.csv"

# Variables (Mantenemos todas para ver cómo las pondera el RBF)
FEATURES = [
    "age", "genero_bin",
...
# =========================
# Ajuste "Realista": Bajamos C a 0.1.
SVM_PARAMS = dict(
    C=0.1,             
    gamma="scale",
    kernel="rbf",      
...
def construir_modelo_SVM(X_train, y_train, cv, ruta_png, ruta_csv):
    modelo = Pipeline(steps=[("scaler", StandardScaler()),("svc", SVC(**SVM_PARAMS))])
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de Aprendizaje de SVM", fontsize=16, weight="bold")
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Curva de pérdida de SVM", fontsize=16, weight="bold")
    plt.xlabel("Tamaño del conjunto de entrenamiento (TRAIN)", fontsize=14)
...
def main():
    ...
    plt.title('Curva ROC Multiclase — SVM (RBF)', fontsize=16, weight='bold')
    ...
    resumen = (
        f"SVM (RBF) — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}, " # <--- F1 Primero
        f"Precisión = {prec_macro:.2f}, Sensibilidad = {rec_macro:.2f}, "
        f"ROC-AUC = {roc_auc_macro:.2f}" if roc_auc_macro is not None
        else f"SVM (RBF) — Accuracy = {acc_test:.2f}, F1-Macro = {f1m_test:.2f}..."
    )
    ...
    ax.set_title("Matriz de confusión: SVM (RBF)", fontsize=16, weight="bold")
...
# ==========================================================
    ...
    RUTA_ROC_JSON = DIR_OUT / "roc_data_svm.json"
