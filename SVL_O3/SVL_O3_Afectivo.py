...
DIR_OUT = Path("SVL_O3/resultados")
DIR_OUT.mkdir(parents=True, exist_ok=True)

# Síntomas que quieres analizar en este OE3 (PHQ-9)
ITEMS = ["phq7", "phq8", "phq9"]
ITEMS_LABELS = {
    "phq7": "Dificultad para concentrarse",
    "phq8": "Agitación",
    "phq9": "Ideacion suicida",
}
...
# Hiperparámetros del SVM lineal (los mismos de tu modelo ganador)
SVM_PARAMS = dict(
    kernel="linear",
    C=1.0,
...
def main():
    ...
    # Para construir OE3 a nivel estudiante
    preds_por_item = {}
    ...
    ruta_tabla = DIR_OUT / "tabla_oe3_phq9_pred_vs_esp.csv"
    ...
    ruta_fig = DIR_OUT / "fig_oe3_phq9_sintomas_pred_vs_esp.png"
    fig.savefig(ruta_fig, dpi=300)
    plt.close(fig)
    print(f"[OK] Figura guardada en: {ruta_fig}")

    # ======================================================
    #   MÉTRICAS GLOBALES DEL OE3 A NIVEL ESTUDIANTE
    # ======================================================

    # Nivel esperado OE3 = máximo de los 3 síntomas (0–3)
    y_oe3_true = df[ITEMS].max(axis=1).astype(int).values

    # Nivel predicho OE3 = máximo de las predicciones de los 3 modelos
    preds_matrix = np.column_stack(
        [preds_por_item[item]["y_pred"] for item in ITEMS]
    )
    y_oe3_pred = preds_matrix.max(axis=1)

    # Probabilidad OE3 = promedio de las probabilidades de los 3 modelos
    proba_stack = np.stack(
        [preds_por_item[item]["proba"] for item in ITEMS], axis=0
    )  # (3, n, 4)
    proba_oe3 = proba_stack.mean(axis=0)  # (n, 4)

    # --- Matriz de confusión ---
    cm = confusion_matrix(y_oe3_true, y_oe3_pred, labels=LIKERT_VALUES)
    ...
    fig_cm.tight_layout()
    ruta_cm = DIR_OUT / "matriz_confusion_oe3_phq9.png"
    fig_cm.savefig(ruta_cm, dpi=300)
    plt.close(fig_cm)
    print(f"[OK] Matriz de confusión guardada en: {ruta_cm}")

    # --- Métricas globales ---
    acc = accuracy_score(y_oe3_true, y_oe3_pred)
    bal_acc = balanced_accuracy_score(y_oe3_true, y_oe3_pred)
    f1_macro = f1_score(y_oe3_true, y_oe3_pred, average="macro")

    try:
        auc_macro = roc_auc_score(
            y_oe3_true, proba_oe3, multi_class="ovr", average="macro"
        )
    except ValueError:
        auc_macro = None

    brier = _brier_multiclass(y_oe3_true, proba_oe3, LIKERT_VALUES)
    ...
    # ---- CSV tipo tabla12_ovr POR SÍNTOMA (solo f00, f01, f10, f11) ----
    df_ovr_multi = None
    for item in ITEMS:
        nombre_bonito = ITEMS_LABELS.get(item, item)
    ...
    ruta_sintomas = DIR_OUT / "metricas_oe3_phq9_por_sintoma.csv"
