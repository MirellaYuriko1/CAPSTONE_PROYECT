...
# Carpeta específica para resultados de SVM (RBF) en el objetivo cognitivo
...
# =========================
# HIPERPARÁMETROS SVM RBF (Idénticos al General)
# =========================
SVM_PARAMS = dict(
    C=0.1,             
    gamma="scale",
...
def make_svm():
    # Pipeline con Scaler es obligatorio para SVM
    return Pipeline([
...
def main():
    ...
    print(f"[INFO] Analizando Dimensión Cognitivo-Afectiva con SVM (RBF) ({len(ITEMS)} ítems).")
    ...
    for item in ITEMS:
        ...
        # Modelo Pipeline (Scaler + SVM RBF)
        clf = make_svm()
    ...
    ax_cm.set_title("Matriz de Confusión: Dimensión Cognitivo-Afectiva (SVM-RBF)", fontsize=12, weight='bold')
    ...
    plt.tight_layout()
    ruta_cm = DIR_OUT / "matriz_confusion_cognitivo_svm.png"
    ...
    df_resumen = pd.DataFrame([{
        "Modelo": "SVM (RBF) (Dimensión Cognitivo-Afectiva)",
