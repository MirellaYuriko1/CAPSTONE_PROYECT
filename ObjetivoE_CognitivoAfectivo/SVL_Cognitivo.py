# SVL_O2/SVL_O2_cognitivo_afectivo.py
...
# Carpeta para resultados de SVM lineal en la dimensión cognitivo-afectiva
DIR_OUT = Path("ObjetivoE_CognitivoAfectivo/resultados_SVL")
DIR_OUT.mkdir(parents=True, exist_ok=True)

# Síntomas que quieres analizar en esta dimensión cognitivo-afectiva (PHQ-9)
...
def main():
    ...
    # Para construir dimensión cognitivo-afectiva a nivel estudiante
    preds_por_item = {}
    ...
    fig.suptitle(
        ...
        f"con respecto a los síntomas cognitivo-afectivos de {sintomas_titulo}.",
        fontsize=12,
        y=0.98,
    )
    ...
    ax_cm.set_title("Matriz de confusión — Dimensión cognitivo-afectiva", fontsize=12)
    ...
    ruta_global = DIR_OUT / "metricas_dimension_cognitivo_afectiva_svl.csv"
    df_global_modelo.to_csv(ruta_global, index=False, encoding="utf-8-sig")
    print(
        "[OK] Métricas globales de la dimensión (SVM lineal, cognitivo-afectiva) "
        "guardadas en:", ruta_global
    )
