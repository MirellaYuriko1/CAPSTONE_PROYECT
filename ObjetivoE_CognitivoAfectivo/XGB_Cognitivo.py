# XGB_O2/XGB_O2_cognitivo_afectivo.py
...
# Carpeta para resultados de XGBoost en la dimensión cognitivo-afectiva
DIR_OUT = Path("ObjetivoE_CognitivoAfectivo/resultados_XGB")
DIR_OUT.mkdir(parents=True, exist_ok=True)

# Síntomas que quieres analizar en esta dimensión (PHQ-9)
ITEMS = ["phq1", "phq2", "phq6", "phq7", "phq8", "phq9"]
ITEMS_LABELS = {
    "phq1": "Estado de ánimo",
    "phq2": "Pérdida de interés",
    "phq6": "Fracaso / inutilidad",
    "phq7": "Dificultad para concentrarse",
    "phq8": "Intranquilidad / agitación",
...
# =========================
# PROCESO PRINCIPAL – DIMENSIÓN COGNITIVO-AFECTIVA
# =========================
def main():
    ...
    # ---------- FIGURA CON 2 ZONAS: CABECERA Y GRÁFICO ----------
    ...
    # Para construir la dimensión a nivel estudiante
    preds_por_item = {}
    ...
    ruta_tabla = DIR_OUT / "tabla_oe2_phq9_pred_vs_esp_xgb.csv"
    ...
    fig.suptitle(
        ...
        f"con respecto a los síntomas cognitivo-afectivos de {sintomas_titulo} (XGBoost).",
        fontsize=12,
        y=0.98,
    )
    ...
    ruta_fig = DIR_OUT / "fig_oe2_phq9_sintomas_pred_vs_esp_xgb.png"
    fig.savefig(ruta_fig, dpi=300)
    plt.close(fig)
    print(f"[OK] Figura guardada en:", ruta_fig)

    # ======================================================
    #   MÉTRICAS GLOBALES DE LA DIMENSIÓN (NIVEL ESTUDIANTE)
    # ======================================================

    # Nivel esperado = máximo de los 6 síntomas (0–3)
    y_oe2_true = df[ITEMS].max(axis=1).astype(int).values

    # Nivel predicho = máximo de las predicciones de los 6 modelos
    preds_matrix = np.column_stack(
        [preds_por_item[item]["y_pred"] for item in ITEMS]
    )
    y_oe2_pred = preds_matrix.max(axis=1)

    # Probabilidad global = promedio de las probabilidades de los 6 modelos
    ...
    ax_cm.set_title("Matriz de confusión — Dimensión cognitivo-afectiva (XGBoost)", fontsize=12)
    ...
    fig_cm.tight_layout()
    ruta_cm = DIR_OUT / "matriz_confusion_oe2_phq9_xgb.png"
    ...
    ruta_sintomas = DIR_OUT / "metricas_oe2_phq9_por_sintoma_xgb.csv"
    ...
    ruta_metricas = DIR_OUT / "metricas_oe2_phq9_xgb.csv"
    df_metricas.to_csv(ruta_metricas, index=False, encoding="utf-8-sig")
    print(f"[OK] Métricas globales guardadas en:", ruta_metricas)

    # ======================================================
    #   CSV RESUMEN GLOBAL PARA LA DIMENSIÓN COGNITIVO-AFECTIVA
    #   (LISTO PARA LA TABLA COMPARATIVA DE LA TESIS)
    # ======================================================
    ...
    ruta_global = DIR_OUT / "metricas_dimension_cognitivo_afectiva_xgb.csv"
    df_global_modelo.to_csv(ruta_global, index=False, encoding="utf-8-sig")
    print(
        "[OK] Métricas globales de la dimensión (XGBoost, cognitivo-afectiva) guardadas en:",
        ruta_global,
    )
