...
# Carpeta específica para resultados de Regresión Logística en el objetivo cognitivo
...
def main():
    ...
    print(f"[INFO] Analizando Dimensión Cognitivo-Afectiva con Regresión Logística ({len(ITEMS)} ítems).")
    ...
    ax_cm.set_title("Matriz de Confusión: Dimensión Cognitivo-Afectiva (LR)", fontsize=12, weight='bold')
    ...
    plt.tight_layout()
    ruta_cm = DIR_OUT / "matriz_confusion_cognitivo_lr.png"
    ...
    df_resumen = pd.DataFrame([{
        "Modelo": "Regresión Logística (Dimensión Cognitivo-Afectiva)",
    ...
    ruta_csv = DIR_OUT / "metricas_cognitivo_lr_tesis.csv"
    df_resumen.to_csv(ruta_csv, index=False, encoding="utf-8-sig")
    print(f"[OK] CSV resumen guardado en: {ruta_csv}")

    print("\n=== RESULTADOS FINALES REGRESIÓN LOGÍSTICA (COGNITIVO-AFECTIVO) ===")
